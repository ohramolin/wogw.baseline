function uc_treeController($scope, $rootScope, $window, $location, $q, uCommerceContentService) {
	...
	$scope.isSelectedCheckbox = function (node) {
		if (node) {
			if ($scope.preSelectedValues) {
				return $scope.preSelectedValues.indexOf(node.id) > -1 && $scope.showCheckBoxForNode(node);
			}
		}
		return false;
	};
		...
		if (node) {
			if ($scope.hasCheckboxFor && node.nodeType != null && node.nodeType != '') {
				...
				for (n in checkBoxTypes) {
					if (checkBoxTypes[n].toLowerCase() == node.nodeType.toLowerCase()) {
						return true;
					}	
				}
			}
	...
	function toggleOptions(node) {
		if ($scope.hasRightClickMenu.toLowerCase() == 'true' && node.options.length > 0) {
